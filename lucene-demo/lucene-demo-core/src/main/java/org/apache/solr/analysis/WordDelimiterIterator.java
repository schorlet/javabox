...
final class WordDelimiterIterator {
     ...
     */
    WordDelimiterIterator(final byte[] charTypeTable, final boolean splitOnCaseChange,
        final boolean splitOnNumerics, final boolean stemEnglishPossessive) {
    ...
    int next() {
        ...
        if (current >= endBounds) return end = DONE;
    ...
    private boolean isBreak(final int lastType, final int type) {
        if ((type & lastType) != 0) return false;

        if (!splitOnCaseChange && isAlpha(lastType) && isAlpha(type)) // ALPHA->ALPHA: always ignore if case isn't
                                                                      // considered.
        return false;
        else if (isUpper(lastType) && isAlpha(type)) // UPPER->letter: Don't split
        return false;
        else if (!splitOnNumerics
            && (isAlpha(lastType) && isDigit(type) || isDigit(lastType) && isAlpha(type))) // ALPHA->NUMERIC,
                                                                                           // NUMERIC->ALPHA :Don't
                                                                                           // split
            return false;
    ...
    boolean isSingleWord() {
        if (hasFinalPossessive) return current == startBounds && end == endBounds - 2;
        else return current == startBounds && end == endBounds;
    }
    ...
    private boolean endsWithPossessive(final int pos) {
        return stemEnglishPossessive && pos > 2 && text[pos - 2] == '\''
            && (text[pos - 1] == 's' || text[pos - 1] == 'S') && isAlpha(charType(text[pos - 3]))
    ...
    private int charType(final int ch) {
        if (ch < charTypeTable.length) return charTypeTable[ch];
