...
public class SslServer implements Runnable {
    private static final Log log = LogFactory.getLog(SslServer.class);
    private SSLContext sc = null;

    public static void main(final String[] args) throws Exception {
        Security.addProvider(new BouncyCastleProvider());

        // System.setProperty("javax.net.ssl.keyStore", "/tmp/localhost.p12");
    ...
    public void run() {
        try {
            ...
            final SSLServerSocket s = (SSLServerSocket) ssf.createServerSocket(8443);
            s.setNeedClientAuth(true);
            if (log.isDebugEnabled()) {
                printServerSocketInfo(s);
            }

            final SSLSocket c = (SSLSocket) s.accept();
            if (log.isDebugEnabled()) {
                printSocketInfo(c);
            }
    ...
    private static void printServerSocketInfo(final SSLServerSocket s) {
        log.debug("Server socket class: " + s.getClass());
        log.debug("   Socker address = " + s.getInetAddress().toString());
        log.debug("   Socker port = " + s.getLocalPort());
        log.debug("   Need client authentication = " + s.getNeedClientAuth());
        log.debug("   Want client authentication = " + s.getWantClientAuth());
        log.debug("   Use client mode = " + s.getUseClientMode());
    }
